...
/** @type {RegExp} Primer comentario HTML (`<!-- -->`) o de bloque C-style. */
const FIRST_COMMENT_RE = /^\s*(?:<!--[\s\S]*?-->|\/\*[\s\S]*?\*\/)\s*/;
...
/**
 * Elimina solo el primer bloque de comentario del código fuente (banner de cabecera).
 * Soporta comentarios HTML (`<!-- ... -->`) y de bloque C-style.
 * @param {string} code
 * @returns {string}
 */
const stripFirstComment = (code) => {
    const match = code.match(FIRST_COMMENT_RE);
    return match ? code.slice(match[0].length) : code;
};
...
export const generateMarkdownShiki = async () => {
    ...
    const results = await Promise.all(
        entries.map(async (entry) => {
            ...
            const code = stripFirstComment(rawCode);
